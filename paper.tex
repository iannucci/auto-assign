...
\subsection{Sub-Segment Traversal Cost}

An ESW must enter and exit each sub-segment at an intersection endpoint. There are four traversal options, each with a different walk distance:

\begin{equation}
c(ss, \text{in}, \text{out}) = \begin{cases}
D & \text{if } \text{in} = S, \text{out} = E \\
D & \text{if } \text{in} = E, \text{out} = S \\
2 \cdot t_n \cdot D & \text{if } \text{in} = \text{out} = S \\
2(1-t_1) \cdot D & \text{if } \text{in} = \text{out} = E
\end{cases}
\label{eq:subseg-cost}
\end{equation}

where $t_1$ and $t_n$ are the positions of the first and last address. Through-traversal ($S \to E$ or $E \to S$) walks the full segment length $D$. Out-and-back ($S \to S$ or $E \to E$) walks only to the farthest address and returns, which is significantly cheaper when addresses cluster near one end.

\subsection{Objective}

Given $M$ sub-segments and $N$ ESWs, find a partition $\{P_1, P_2, \ldots, P_N\}$ of the sub-segments, an ordering within each partition, and entry/exit choices for each sub-segment, that minimize:
...
\begin{equation}
W(P_k) = \underbrace{\sum_{i=1}^{|P_k|} c(ss_i, \text{in}_i, \text{out}_i)}_{\text{productive (intra-sub-segment)}} + \underbrace{\sum_{i=1}^{|P_k|-1} d_{\text{road}}(\text{out}_i, \text{in}_{i+1})}_{\text{unproductive (inter-sub-segment)}}
\label{eq:total-walk}
\end{equation}

The first sum is \emph{productive} walking (visiting addresses). The second sum is \emph{unproductive} walking (traveling between sub-segments along the road network). A good assignment minimizes unproductive walking.

The balance constraint requires:
\begin{equation}
\frac{\max_k |P_k|}{\min_k |P_k|} \leq \beta
\end{equation}
where $|P_k|$ is the total number of addresses in ESW $k$'s assigned sub-segments and $\beta \approx 1.0$.

\subsection{Special Cases}

\textbf{Paired sub-segments.} When an ESW is assigned both sides of the same segment, the optimal traversal is the U-turn: walk $S \to E$ on one side, cross the street at $E$ (negligible distance), walk $E \to S$ on the other side. Total: $2D$ with zero unproductive distance. This U-turn pattern, which our earlier study found to reduce walk distance by 33--71\%, arises naturally from the sub-segment formulation.

\textbf{Cul-de-sacs.} When $S = E$ (dead-end street), through-traversal and out-and-back are equivalent. The ESW enters and exits at the single intersection.
...
Addresses come from the City of Palo Alto municipal address database (44,115 addresses), stored in PostGIS with WGS84 coordinates. Each address is snapped to its nearest road segment (within 200\,m) and assigned to the left or right sub-segment by the cross-product of the road direction vector and the address displacement vector.

Inter-sub-segment distances $d_{\text{road}}$ are computed as shortest paths on the road network graph using Dijkstra's algorithm, precomputed between all intersection pairs within each neighborhood.
